# Pinecone bot - deterministyczny stos sterowania

Nowy, deterministyczny tor sterowania robotem-zbieraczem szyszek. Zyje w
`pinecone_bot/` obok starych skryptow zespolu (`approach_and_grasp.py`,
`drive_to_target.py`, `web_control.py` itd.) i ich nie zastepuje ani nie
zmienia.
...
1. **Zaleznosci.** `uv pip install -r requirements-pi.txt` (albo
   `requirements-pinecone.txt`, patrz sekcja nizej) w wenwie repo. Szczegoly
...
5. **Nagraj chwyty ramieniem:**
   ...
   ```
   UWAGA: `motions/grasp_mid.json` dolaczony do repo to tylko PRZYKLAD
   formatu pliku, nagrany PRZED korekta barku (`fix_shoulder_offset.py`,
   2026-09-25) - trzeba go nagrac od nowa, tak jak pozostale chwyty.
